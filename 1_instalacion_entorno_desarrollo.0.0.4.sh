...
# Cambios en 0.0.4:
#              - Firefox instalación con método oficial de Mozilla (APT + GPG key verificada)
#              - Desinstalación mejorada: protege GNOME y herramientas críticas del sistema
#              - Remueve gnome-keyring solo si se desinstala por completo (todavía no lo hace)
#              - Mantiene estabilidad del entorno de escritorio GNOME
# Cambios en 0.0.3:
#              - Método de instalación de VS Code actualizado a .deb directo (code.visualstudio.com)
#              - Usa curl + apt install local para evitar problemas de repo APT
#              - Conserva desinstalación completa y pruebas de VM
# Cambios en 0.0.2:
...
install_firefox() {
    ...
    print_info "Instalando Firefox siguiendo método oficial de Mozilla..."
    
    # Paso 1: Crear directorio de keyrings si no existe
    print_info "Creando directorio de keyrings..."
    sudo install -d -m 0755 /etc/apt/keyrings >/dev/null 2>&1
    
    # Paso 2: Descargar e importar la clave GPG de Mozilla
    print_info "Descargar clave GPG de Mozilla..."
    if ! wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O- | sudo tee /etc/apt/keyrings/packages.mozilla.org.asc > /dev/null 2>&1; then
        print_error "Error al descargar la clave GPG de Mozilla"
        return 1
    fi
    
    # Paso 3: Verificar el fingerprint de la clave
    print_info "Verificando fingerprint de la clave GPG..."
    local expected_fingerprint="35BAA0B33E9EB396F59CA838C0BA5CE6DC6315A3"
    local actual_fingerprint=$(gpg -n -q --import --import-options import-show /etc/apt/keyrings/packages.mozilla.org.asc 2>/dev/null | awk '/pub/{getline; gsub(/^ +| +$/,""); print}')
    
    if [[ "$actual_fingerprint" == "$expected_fingerprint" ]]; then
        print_success "Fingerprint de clave GPG verificado correctamente"
    else
        print_error "El fingerprint de la clave GPG no coincide. Esperado: $expected_fingerprint, Obtenido: $actual_fingerprint"
        return 1
    fi
    
    # Paso 4: Agregar repositorio de Mozilla (para Debian Bookworm)
    ...
    # Paso 5: Configurar prioridad de APT para repositorio de Mozilla
    print_info "Configurando preferencias de APT..."
    cat <<EOF | sudo tee /etc/apt/preferences.d/mozilla >/dev/null 2>&1
Package: *
Pin: origin packages.mozilla.org
Pin-Priority: 1000
EOF
    
    # Paso 6: Actualizar índices e instalar Firefox
    print_info "Actualizando índices de paquetes..."
    ...
    if sudo apt install -y -qq firefox; then
        ...
        print_success "Firefox instalado correctamente: $FIREFOX_VERSION"
        log_message "Firefox instalado desde repositorio oficial de Mozilla"
    else
...
install_vscode() {
    ...
    print_info "Instalando Visual Studio Code desde paquete .deb oficial..."
    local tmp_deb="/tmp/vscode-linux-x64.deb"

    if sudo rm -f "$tmp_deb" >/dev/null 2>&1; then
        true
    fi

    if ! curl -L -o "$tmp_deb" "https://code.visualstudio.com/sha/download?build=stable&os=linux-deb-x64"; then
        print_error "Error al descargar el paquete .deb de Visual Studio Code"
        return 1
    fi

    print_info "Instalando paquete .deb de VS Code..."
    if sudo apt install -y -qq "$tmp_deb"; then
        VSCODE_VERSION=$(code --version 2>/dev/null | head -1 || echo "versión desconocida")
        print_success "Visual Studio Code instalado: $VSCODE_VERSION"
        log_message "VS Code instalado correctamente desde .deb"
    else
        print_warning "Error al instalar .deb inicial. Intentando resolver dependencias..."
        sudo apt --fix-broken install -y -qq
        if sudo apt install -y -qq "$tmp_deb"; then
            VSCODE_VERSION=$(code --version 2>/dev/null | head -1 || echo "versión desconocida")
            print_success "Visual Studio Code instalado tras corregir dependencias: $VSCODE_VERSION"
            log_message "VS Code instalado correctamente desde .deb tras apt --fix-broken"
        else
            print_error "No se pudo instalar Visual Studio Code desde .deb"
            return 1
        fi
    fi
...
uninstall_build_tools() {
    print_info "Desinstalando herramientas de compilación (NO se desinstalará gnome-keyring)..."
    # IMPORTANTE: No desinstalar gnome-keyring ya que es crítico para GNOME
    # y su remoción desestabiliza el entorno de escritorio (startx no funciona correctamente)
    if sudo apt remove -y -qq build-essential curl wget git-lfs; then
        print_success "Herramientas de compilación desinstaladas"
        log_message "Build-tools desinstalados (gnome-keyring protegido)"
    else
